...
from machine_learning_project.models.package_artifacts import load_packaging_run
...
def audit_calls():
            ...
            if "model" in self.named_steps:
                ...
                if expected is None or not x.equals(expected.loc[x.index]):
                    raise AssertionError(
                        "Inference batch is outside the declared validation/synthetic population"
                    )
                counts = state["counts"].setdefault(
                    state["phase"], {"predict": 0, "predict_proba": 0, "rows": 0}
                )
                counts[name] += 1
                counts["rows"] += len(x)
    ...
    with ExitStack() as stack:
        ...
        stack.enter_context(
            patch.object(Pipeline, "predict_proba", spy(Pipeline.predict_proba, "predict_proba"))
        )
        yield state
...
def isolated_check(package, expected, workspace):
    ...
    with tempfile.TemporaryDirectory(prefix="isolated-runtime-", dir=workspace) as directory:
        ...
        command = [
            ...
            "--target",
            str(installed),
            str(clone / package.manifest["wheel"]),
        ]
        ...
        settings = {
            ...
            "forbidden": [
                str(root / p) for p in ("src", "pipelines", "data", "reports", "artifacts/models")
            ],
        }
        ...
        invocation = [
            ...
            "-I",
            "-S",
            str(directory / "check.py"),
            str(directory / "settings.json"),
        ]
        result = subprocess.run(
            invocation, cwd=directory, capture_output=True, text=True, check=False
        )
        if result.returncode:
            raise AssertionError("Isolated runtime failed: " + result.stderr)
        evidence = json.loads(result.stdout)
        if evidence["prediction_sha256"] != fingerprint(expected) or evidence["counts"] != {
            "fit": 0,
            "predict": 3,
            "predict_proba": 3,
        }:
...
def verify_packages(inputs, training_config, prefix, workspace):
    ...
    with audit_calls() as audit:
        ...
        train, validation, identity = prepare_benchmark_partitions(
            train, validation, data, training_config, provenance
        )
        del train
        ...
        for family in inputs["packaging_config"]["families"]:
            ...
            request = validate_request(
                validation[["content_id", *package.schema["features"]]],
                package.schema,
                package.config,
            )
            ...
            digest = _hash_value(
                fingerprint(list(labels)), f"content-trend-tuning-v1:{provenance['source_sha256']}"
            )
            ...
            for run in (one, two):
                ...
                if [r["predicted_trend"] for r in label_only["records"]] != list(labels) or any(
                    r["probabilities"] is not None for r in label_only["records"]
                ):
                ...
                observed = np.asarray(
                    [
                        [r["probabilities"][str(c)] for c in source.classes_]
                        for r in with_probabilities["records"]
                    ]
                )
            ...
            evidence["parity"][family] = {
                "row_count": len(request),
                "partition": "validation",
                "prediction_sha256": digest,
                "exact_labels": True,
                "probabilities_match_atol": 1e-9,
                "model_bytes_unchanged": sha256_file(package.root / "model.joblib")
                == initial["references"]["model_hashes"][f"{family}.joblib"],
            }
            ...
            for proba in (False, True):
                ...
                modes[str(proba)] = {
                    "prediction_including_validation_seconds": predict_seconds,
                    "serialization_seconds": serialization_seconds,
                    "serialized_bytes": len(serialized.encode()),
                    "rows": response["row_count"],
                }
            evidence["workload"][family] = {
                "source": "invented_synthetic_rows_only",
                "row_count": 30000,
                "cold_load_seconds": load_seconds,
                "validation_seconds": validation_seconds,
                "modes": modes,
                "chunk_rows": package.config["chunk_rows"],
                "sla_certified": False,
            }
            tiny = synthetic_request(package.schema, 17)
            audit.update(
                phase="isolated_smoke_reference",
                expected=validate_request(tiny, package.schema, package.config)[
                    package.schema["features"]
                ],
            )
...
def main():
    ...
    inputs = {
        "packaging_config": load_yaml("configs/packaging.yaml")["packaging"],
    ...
    evidence, roots = verify_packages(
        inputs, load_yaml("configs/training.yaml")["training"], args.run_prefix, ".packaging-work"
    )
    evidence.update(
        checks=checks,
